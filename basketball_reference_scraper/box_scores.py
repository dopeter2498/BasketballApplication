...
def get_all_star_box_score(year: int):
    ...
    if r.status_code == 200:
        ...
        team_names = list(
            map(lambda el: el.text, soup.select(
                "div.section_heading > h2")[1:3])
        )
...
def get_schedule(year: int, month=""):
    league = "NBA"
    if year < 1949:
        league = "BAA"
    r = None
    if month != "" and month.lower() != "october":
        r = get(
            f"https://widgets.sports-reference.com/wg.fcgi?css=1&site=bbr&url=%2Fleagues%2F{league}_{year}_games-{month.lower()}.html&div=div_schedule"
        )
    else:
        r = get(
            f"https://widgets.sports-reference.com/wg.fcgi?css=1&site=bbr&url=%2Fleagues%2F{league}_{year}_games.html&div=div_schedule"
        )
    df = None
    if r.status_code == 200:
        soup = BeautifulSoup(r.content, "html.parser")
